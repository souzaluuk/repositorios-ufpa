...
function shareItem(socialNetwork, title, titlePopUp) {
	...
	var paramsPopUp = 	'width=740, height=740, toolbar=no, scrollbars=yes, status=yes, maximized=no';
	var mainURL 	=	"";
	var titlePopUp  = 	"";
	...
	switch( socialNetwork ) {
		case 'twitter':
			mainURL 	= "https://twitter.com/intent/tweet?text=" + title + " - " + "&url=" + sharedURL;
			break;
			
		case 'facebook':
			mainURL 	= "https://www.facebook.com/sharer/sharer.php?" +  "u=" + sharedURL;
			break;
		
		case 'google':
			mainURL		= "https://plus.google.com/share?app=110&url=" + sharedURL;
			break;

		case 'linkedin':
			mainURL		= "https://www.linkedin.com/cws/share?url=" + sharedURL;			
			
			break;
		case 'email':
			mainURL		= "<%= request.getContextPath() %>/suggest?handle=<%= handle %>";
			paramsPopUp = 'width='+screen.width+', height='+screen.height+', toolbar=no, scrollbars=yes, status=yes, maximized=no';
			break;
		case 'print':
			window.print();
			return;
...
/**
 * FunÃ§Ã£o de comentÃ¡rio do facebook
 * @param d
 * @param s
 * @param id
 */
(function(d, s, id) {
  ...
  fjs.parentNode.insertBefore(js, fjs);
				...
				<%
						String hanle_page = HandleManager.getCanonicalForm(handle);
				%>	
                <%-- <strong>Please use this identifier to cite or link to this item:
                <code><%= hanle_page %></code></strong>--%>
                <div class="well">
				<h5>
					<fmt:message key="jsp.display-item.identifier"/>
					<code><%= hanle_page %></code>
				</h5>
				<div>
					<h5>Compartilhar:  
					<!--Botão Facebook-->
					<img class="social-pointer" title="Compartilhar no Facebook" src="/jspui/image/social-facebook.png" onclick="shareItem('facebook', '', 'Compartilhar no Facebook')">
					<!--Botão Facebook-->
					
					<!--Botão Twitter-->
					<img class="social-pointer" title="Compartilhar no Twitter" src="/jspui/image/social-twitter.png" onclick="shareItem('twitter', '', 'Compartilhar no Twitter')">
					<!--Botão Twitter-->
					
					<!--Botão G+-->
					<img class="social-pointer" title="Compartilhar no G+" src="/jspui/image/social-google.png" onclick="shareItem('google', '', 'Compartilhar no G+')">
					<!--Botão G+-->
					
					<!--Botão LinkedIn-->
					<img class="social-pointer" title="Compartilhar no LinkedIn" src="/jspui/image/social-in.png" onclick="shareItem('linkedin','','Compartilhar LinkedIn')">
					<!--Botão Linkedin-->
					
					<!--Botão Print-->
					<img class="social-pointer" title="Imprimir Página" src="/jspui/image/social-print.png" onclick="shareItem('print','','Imprimir Página')">
					<!--Botão Print-->
					
					<!--Botão Email-->
					<img class="social-pointer" title="Compartilhar por Email" src="/jspui/image/social-email.png" onclick="shareItem('email','','Compartilhar Email')">
					<!--Botão Email-->
					</h5>
				</div>
				</div>
